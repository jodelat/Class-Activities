...
// 	var swaps = true;//swaps is used to see if we have done any
// 	//swaps during our iteration of the array
// 	while (swaps) {//as long as we have swapped, continue looping
// 		swaps = false;//reset swaps to false before iterating array
// 		//loop from i = 0 to i = nums.length - 2 (index before last index)
// 		//we want to stop early because we compare i to i + 1 so if we go
// 		//to the very last index i + 1 would be undefined.
// 		for(var i = 0; i + 1 < nums.length; i++) {
// 			if (nums[i] > nums[i+1]) {//if two items are out of order'
// 				console.log('going to swap ' + nums[i] + ' and ' + nums[i+1])
// 				//we are going to swap so set swaps to true.
// 				swaps = true;
// 				//swap them using a temporary variable:
// 				var temp = nums[i];//store nums[i] before
// 				nums[i] = nums[i+1];//replacing it with nums[i+1]
// 				nums[i+1] = temp;//then replace nums[i+1] with temp
// 			}
// 		}
// 		console.log('after one complete iteration:');
// 		displayArray(nums);
// 	}
